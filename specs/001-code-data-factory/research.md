...
**Revision**: 2.0 | **Research date**: 2026-09-08 | **Method evidence cutoff**: 2026-04-01

本文取代旧代码任务选型。结论是本项目设计选择；外部接口可读不代表本地兼容性通过。所有组件
...
**Decision**: 信息查询、计算、单位与时间转换；冻结资料与受限工具。Toucan 的历史轨迹先进入
观察/审计层，只有依赖完整且重新验证后才进可执行层；公开资料上的派生任务重新生成与验证。
...
范围、主要组件、唯一数据干预、RL 适配入口和长程验收已确定，无待澄清的产品范围。
尚待实施的证据：100 任务先导、组件组合锁定、真实采样、隔离执行、多节点、SFT 与数据收益。
