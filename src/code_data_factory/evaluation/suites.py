...
def _task(index: int, *, split: Literal["DEVELOPMENT", "TEST"], group_count: int, unseen: bool) -> EvaluationTask:
    ...
    return EvaluationTask(
        ...
        instruction=f"Use only the available tools to determine {value} units from {document_ids[0]} and cite the document.",
        document_ids=document_ids, expected_value=value, expected_unit=unit,
    )
...
def load_suite(path: Path) -> EvaluationSuite:
    ...
    def load(split: Literal["DEVELOPMENT", "TEST"], records: object) -> tuple[EvaluationTask, ...]:
        ...
        for record in records:
            if not isinstance(record, dict) or any(key not in record for key in ("task_id", "family", "group_id", "template_family_id", "dependency_depth", "tool_signature", "unseen_combination", "recovery_condition", "instruction", "document_ids")):
